...
const App = () => {
  ...
  const { isFullscreen, homeScreenStatus, handleAddToHomeScreen, miniApp } = useTMALogic();
  const {
    cart,
    handleAddToCart,
  ...
  }, [clearWelcomeTimers, handleDismissWelcome, isLoading, markWelcomeSeen, welcomeRendered, setWelcomeRendered, setShowWelcome]);
  ...
  }, [cart, paymentMethod, appliedPromo, discount, subtotal, deliveryPrice, total, deliveryCity, orderHistory, setOrderHistory, miniApp, clearCart, setCartOpen, setShowConfirmation, showNotification]);
  ...
  const handleToggleWishlist = useCallback(async (product) => {
    ...
    if (isInWishlist) {
      ...
      showNotification(`${product.emoji} ${product.name} retiré des favoris`, 'info');
    } else {
      newWishlist = [...wishlist, product];
      showNotification(`${product.emoji} ${product.name} ajouté aux favoris`, 'success');
    }
